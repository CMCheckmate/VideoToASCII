...
function setResolution(manual = false) {
    ...
    if (!manual || (widthInput.value == "0" && heightInput.value == "0")) {
        ...
        const maxWidth = Math.min(frameBox.offsetWidth, window.innerWidth) * FRAME_SIZE;
        const maxHeight = Math.min(frameBox.offsetHeight, window.innerHeight) * FRAME_SIZE;
...
function setFrames(repeat = true) {
    ...
    for (let i = 0; i < pixels.length; i += 4) {
        ...
        if ((!inverted && pixelBrightness > COLOUR_THRESHOLD) || (inverted && pixelBrightness < COLOUR_THRESHOLD)) {
...
function handleFileUpload(event) {
    ...
    if (fileInput) {
        ...
        if (fileInput == DEFAULT_VIDEO) {
            videoURL = fileInput;
            videoName = fileInput.split("/").pop().split(".")[0];
        } else {
            videoURL = URL.createObjectURL(fileInput);
            videoName = fileInput.name.match(/^[^.]+/);
        }
        video.src = videoURL;
        playState = "loading";
        ...
        video.addEventListener("loadeddata", function() {setupVideo(true);});
        video.addEventListener("ended", function() {setupVideo(false);});
        setTimeout(function() {if (!video.duration) {document.getElementById("frameText").innerHTML = "Cannot load file";}}, 1000 * MAX_LOAD_TIME);
    }
}
